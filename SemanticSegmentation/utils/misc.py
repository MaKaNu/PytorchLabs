...
"""Utility Functions and Classes: Miscellaneous"""
...
def check_mkdir(dir_name:str) -> None:
    """Creates the directory "dir_name" if the path does not exists.

    Args:
        dir_name (str): Name of directory which is checked
...
def initialize_weights(*models:list) -> None:
    """Goes over all models in "models" and picks their modules.
    Uses Kaiming_normal for Conv2D and Linear.
    Fills with 1 for BatchNorm2D
    Fills with 0 for Bias

    Args:
        models (list, elementwise): Model(s) which should be initialzed
...
def get_upsampling_weight(
    in_channels:int, out_channels:int, kernel_size:int) -> torch.Tensor:
    """Calculates Upsampling Weights for a given count of in-/output channels
    based on kernel_size

    Args:
        in_channel (int): Count of Input Channels of Layer
        out_channel (int): Count of Output Channels of Layer
        kernel_size (int): Size of the upsampling Conv Kernel
    ...
    if kernel_size % 2 == 1:
        ...
        center = factor - 0.5
    mesh = np.ogrid[:kernel_size, :kernel_size]
    filt = (1 - abs(mesh[0] - center) / factor) * \
           (1 - abs(mesh[1] - center) / factor)
    weight = np.zeros(
...
class CrossEntropyLoss2d(nn.Module):
    """ Loss Module which uses the call method of NLLLoss2d to calculate
    log_softmax on it.
    NLLLoss2d Class is deprecated use NLLLoss instead.

    Attributes:
        nll_loss (NLLLoss2d): class object to calculate Negative log Likelihood
"""

    def __init__(
        self, weight:torch.Tensor = None, size_average:bool = True,
        ignore_index:int = 255) -> None:
        """ Description

        Args:
            weight (torch.Tensor): a manual rescaling weight given to each
                class. If given, it has to be a Tensor of size C. Otherwise, it
                is treated as if having all ones.
            size_average (bool): Deprecated (see reduction). By default, the
                losses are averaged over each loss element in the batch. Note
                that for some losses, there are multiple elements per sample.
            ...
            ignore_index (int): Specifies a target value that is ignored and
                does not contribute to the input gradient. When size_average is
    ...
    def forward(
        self, inputs:torch.Tensor, targets:torch.Tensor) -> nn.NLLLoss2d:
        """overridden forward Function.

        Args:
            inputs (Tensor): Input Tesnor
